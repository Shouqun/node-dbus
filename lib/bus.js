...
var Bus = module.exports = function(dbus, busName) {
	...
	self.on('signal', function(uniqueBusName, serviceName, objectPath, interfaceName, interfaceObj, signalName, args) {
		var signalHash = serviceName + ':' + objectPath + ':' + interfaceName;

		if (self.signalHandlers[signalHash]) {
			var args = [ signalName ].concat(Array.prototype.slice.call(arguments));

			self.signalHandlers[signalHash].emit(self.signalHandlers[signalHash], args);
		}
	});
...
Bus.prototype.registerSignalHandler = function(serviceName, objectPath, interfaceName, interfaceObj) {
	...
	var signalHash = serviceName + ':' + objectPath + ':' + interfaceName;
	if (!self.signalHandlers[signalHash])
		self.signalHandlers[signalHash] = [];

	// Register interface signal handler
	self.signalHandlers[signalHash].push(interfaceObj);
};
